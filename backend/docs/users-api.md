...
별도 boolean·완료 시각을 저장하거나 사용자가 직접 완료 상태를 수정하는 API는 없다. 선호 운동·운동 목적은 계획에 없으며 완료 조건도 아니다. 온보딩 완료는 체력 평가·추천 데이터가 충분하다는 뜻이 아니다. **이 조건은 후속 회의에서 재검토할 MVP 정책**이다. 기존 측정 API의 부분 저장·소유권·Decimal 문자열·생성 요청 키·revision 계약은 유지한다.
...
후속 범위: 과거 대비 변화/성장, 운동 콘텐츠·추천 알고리즘, 재화 지급·차감·구매·교환·거래 이력, 추천 호출 화면/시점, 온보딩 조건 재검토. 선호 운동·운동 목적·개인별 목표/기준값은 후속 예정 기능으로 두지 않는다.
