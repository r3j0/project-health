...
구현일: 2026-09-21. 이미지 판독 보완·공식 문서 확인일: 2026-09-22. 만 13–64세의 국민체력100 결과지에서 한 사람·한 측정 회차를 추출한다. 별도 OCR 엔진이나 팀원 서버 없이 **우리 백엔드에서 OpenAI Responses API를 호출**한다. 팀원의 키도 서버 비밀 설정으로만 주입한다.
...
| 이미지 입력 | `input_image`, 인라인 PNG data URL, `detail: auto`, 원본 + 상세 이미지 최대 4장                        |
| 외부 호출   | 네이티브 fetch, 요청당 최대 1회, 자동 재시도 없음, 리다이렉트 금지                                     |
| 시간 제한   | 인증·호출 제한 검사 후 업로드부터 결과 반환까지 75초, 이미지 변환별 8초, 외부 연결·본문 수신 합계 60초 |
...
작은 숫자를 읽기 위해 원본 크기를 유지한 전체 이미지와 겹치는 네 구간의 상세 이미지를 같은 요청에 보낸다. 짧은 변 512픽셀 이상이고 긴 변 1,200픽셀 이상인 이미지에만 상세 이미지를 추가한다. 각 구간은 원본 가로·세로의 60%이며 중앙에서 20% 겹친다. 보간 확대는 최대 2배, 각 상세 이미지의 긴 변은 최대 2,400픽셀, 전체 준비 이미지의 합계는 24 MiB로 제한한다. 사진 내용을 생성하거나 측정값을 보정하지 않는다. 상세 이미지도 메모리에만 존재하며 실제 이미지 작업 종료 시 비운다. 한 번의 외부 호출에 이미지가 여러 장 포함되므로 입력 토큰 비용은 전체 한 장보다 증가할 수 있다.

연결 종료·요청 타임아웃으로 HTTP 응답 처리가 끝나도, 이미 시작한 이미지 변환이 종료되기 전에는 동시 처리 슬롯을 반환하거나 사용 중인 버퍼를 비우지 않는다. 아직 정리 중인 작업까지 4개라면 새 요청에 `503 EXTRACTION_BUSY`를 반환한다. 업로드 도중 타임아웃은 504 응답을 전송한 뒤 미완료 요청 스트림을 종료해 multipart 파서와 슬롯을 정리한다.

[OpenAI 이미지 입력 문서](https://developers.openai.com/api/docs/guides/images-vision)에 따라 `auto`를 사용한다. 실제 검증 모델인 `gpt-5.6-terra`에서는 원본 크기를 유지하며, 다른 모델은 해당 모델의 기본 해상도 정책을 따른다. 상세 이미지가 같은 사진의 일부임을 명시해 중복 후보를 만들지 않도록 요청한다. 원본 범례와 본인 막대의 인쇄된 숫자를 함께 확인하고, 기준 막대·축 눈금으로 값을 추정하지 않는다.
...
Sharp가 프레임 수를 노출하지 않는 APNG도 PNG 청크 경계와 `acTL`의 프레임 수를 검사해 다중 프레임이면 `400 MULTI_FRAME_IMAGE`로 거절한다. 일반 PNG의 텍스트나 파일 끝에 붙은 문자열을 애니메이션 청크로 취급하지 않는다.
