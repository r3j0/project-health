...
다음 JSON의 `catalogVersion`은 위 응답의 version을 사용한다. 아래 버전·날짜·측정값은 재현용 예시이며, 운영 서비스가 생성하는 사용자 결과가 아니다. 최신 카탈로그 버전은 `nfa100-2026-09-21-self-v1`이며 이전 `nfa100-2026-09-19`도 조회·입력할 수 있다. 직접 측정값으로 바꿔도 같은 검증·저장 경로를 거친다.
...
생성 시 `entryMethod: "self_assessment"`를 지정하면 만 19~64세 자가측정으로 저장한다. 생략하거나 `manual`이면 기존 직접 입력 계약이 유지된다. PATCH는 저장 출처를 바꿀 수 없다. 기존 POST의 멱등 키는 생략/명시한 `manual`을 같은 요청으로 처리한다.

자가측정은 `reportKind: "simple"`, 인증등급 없음, 실제 측정값 1개 이상을 요구한다. 신장·체중·BMI·허리둘레·교차 윗몸일으키기·성인 윗몸말아올리기·YMCA 회복 심박수·앉아 윗몸 앞으로 굽히기만 허용한다. 근지구력 두 항목은 동시에 저장하지 않는다. 미측정 항목은 보내지 않는다. 나이/항목/유형/등급 수정 시에도 동일한 검증이 적용된다.

새 카탈로그는 이전 정의 19개를 유지하고 다음 두 정의를 추가한다. 이전 버전과 기존 기록은 변경하지 않는다.

| 코드                       | 값                | 구분                                          |
| -------------------------- | ----------------- | --------------------------------------------- |
| `self_curl_up`             | 0 이상의 정수, 회 | 성인 자가측정; 청소년 `curl_up`과 별도        |
| `ymca_recovery_heart_rate` | 0 초과 정수, bpm  | 10초 맥박 횟수 × 6; `step_test_vo2max`와 별도 |

공식 프로토콜: [국민체력100 간이측정](https://nfa.kspo.or.kr/measure/self/selectSelfMeasureItem.kspo). 교차 윗몸일으키기는 60초, YMCA는 30cm 박스에서 96bpm으로 180초 후 60초 회복·10초 맥박 측정이다. BMI는 프론트엔드에서 신장·체중이 모두 있을 때 kg/m²로 소수 1자리 계산한다. 저장된 수치는 인증 점수·등급·운동 추천으로 변환하지 않는다.
