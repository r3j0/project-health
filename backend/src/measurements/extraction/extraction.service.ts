...
export class ExtractionService {
  ...
  async extract(
    ...
    let views: ExtractionImageView[] = [{ image, label: '원본 전체' }];
    try {
      ...
      if (signal.aborted) throw extractionError(504, 'EXTRACTION_TIMEOUT');
      views = await prepareImageViews(image, signal);
      return validateExtraction(
        await this.openai.extract(views, catalog, signal),
        catalog,
      );
    } finally {
      for (const view of views) view.image.fill(0);
    }
  }
}
