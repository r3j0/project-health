...
- 기존에 저장된 `self_curl_up` 기록은 조회·수정 시 보존한다. 이전 간이측정 초안의 해당 횟수는 다른 종목으로 변환하지 않고 제외하며 다른 측정값은 유지한다. 서버에 이미 전달했을 수 있는 미확정 저장 요청은 원래 키·본문으로 재확인한다. 백엔드 카탈로그·스키마·API는 변경하지 않았다.
...
2. YMCA bpm은 현재 서버에서 호환 공식 기준 미확보로 반환한다. 값은 저장하고 평가 불가 사유를 표시한다. 전달받은 환산식의 측정 조건 확인과 백엔드 적용은 별도 작업이다. 성인 `self_curl_up`의 신규 등록은 제거했으며 과거 기록의 평가 사유는 서버 응답대로 표시한다.
