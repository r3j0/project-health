...
- `/account/preferences`: 운동량(더 적게/기본/더 많이)과 운동 목적(등급 개선/체형 관리/기본 체력 증진)의 단일 선택 UI. 현재는 화면 기틀만 제공하며 API 조회·저장이나 브라우저 영속 저장을 하지 않습니다. 저장 버튼은 준비 중 안내와 함께 비활성화하며 화면 이탈 시 선택이 초기화됩니다. 초기 화면 값은 운동량 `standard`, 목적 미선택으로, 저장된 사용자 설정을 의미하지 않습니다.
