...
화면 상태·사용자 프로필·커리큘럼과 독립적입니다. `framing="face"`는 같은 호흡 엔진으로 얼굴만 표시합니다. 기본 `framing="full"`은 전신입니다. `variant`, `motion`, `cycleSeconds`, `intensity`, `paused`, `label`, `size`로 제어합니다. 장식용으로 사용할 때는 `label=""`을 전달합니다.
